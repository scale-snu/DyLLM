...
from ._shim import envs, current_platform, init_logger, KVQuantMode
import triton
...
def kernel_unified_attention(
    ...
    if USE_PER_SEQ_CAUSAL:
        _is_causal_seq = tl.load(per_seq_causal_ptr + seq_idx)
    else:
        _is_causal_seq = True
    ...
    for j in range(loop_lo, loop_hi):
        ...
        if SLIDING_WINDOW:
            ...
            if USE_PER_SEQ_CAUSAL:
                ...
                # DyLLM-DG: bidirectional rows ignore the window (trimmed KV)
                sw_mask_v = (dist < SLIDING_WINDOW) | (is_causal_seq == 0)
            elif USE_CAUSAL:
...
# Local: launch-config override for tuning probes
# ({"block_m", "num_warps", "num_stages", "tile_prefill"}); empty = kernel defaults.
LAUNCH_OVERRIDE: dict[str, int] = {}
...
def unified_attention(
    ...
    # Local (A100, sm80): the decode-oriented defaults under-tile the canvas /
    # prefill shapes we run (BLOCK_Q 8 for D256, 2 for D512). Sweep on our
    # geometries (DyLLM-tests/probe_triton_tune.py): ~2x for both head sizes.
    tuned_sm80 = (
    ...
    # Same sweep on H100 (sm90): D256 takes a wider query block than sm80,
    # D512 lands on the sm80 config. 1.8x (D512) over the decode defaults.
    tuned_sm90 = (
        head_size in (256, 512)
        and max_seqlen_q > 1
        and num_queries_per_kv <= 16
    ...
    if tuned_sm90:
        BLOCK_M = 128 if head_size == 256 else 64
        BLOCK_Q = BLOCK_M // num_queries_per_kv
        launch_num_warps = 8
        launch_num_stages = 2
    if LAUNCH_OVERRIDE:
        BLOCK_M = LAUNCH_OVERRIDE.get("block_m", BLOCK_M)
        BLOCK_Q = BLOCK_M // num_queries_per_kv
        launch_num_warps = LAUNCH_OVERRIDE.get("num_warps", launch_num_warps)
        launch_num_stages = LAUNCH_OVERRIDE.get("num_stages", launch_num_stages)
    ...
    if (tuned_sm80 or tuned_sm90) and head_size == 256:
        TILE_SIZE_PREFILL = 64
    if LAUNCH_OVERRIDE.get("tile_prefill"):
        TILE_SIZE_PREFILL = LAUNCH_OVERRIDE["tile_prefill"]
