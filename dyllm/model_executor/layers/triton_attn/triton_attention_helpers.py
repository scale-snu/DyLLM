...
def compute_tile_loop_bounds(
    ...
    is_causal_seq=True,  # runtime; only read under USE_PER_SEQ_CAUSAL
    ...
    if SLIDING_WINDOW > 0 and not USE_MM_PREFIX:
        ...
        if USE_PER_SEQ_CAUSAL:
            # DyLLM-DG: bidirectional rows ignore the window (whole trimmed KV)
            tile_start = tl.where(is_causal_seq, tile_start, 0)
            tile_end = tl.where(is_causal_seq, tile_end, num_tiles)
...
def compute_kv_seq_mask(
    ...
    if CHUNK_LOOKBACK > -1:
        ...
        if USE_PER_SEQ_CAUSAL:
            # DyLLM-DG: bidirectional (denoise) rows attend the whole trimmed
            # KV; the window applies to causal (encode) rows only.
            seq_mask = seq_mask & (sw_left | (is_causal == 0))
        elif not USE_CAUSAL:
    ...
    # within the sliding window: (causal OR blockwise) AND sliding_window.
    # We AND each range with the SW past-bound
    # (query_abs_pos - seq_offset) < SLIDING_WINDOW (kv > q - sw; future kv
    # passes trivially). Inert for full-attention layers
    # (SLIDING_WINDOW <= 0).
    if USE_MM_PREFIX:
